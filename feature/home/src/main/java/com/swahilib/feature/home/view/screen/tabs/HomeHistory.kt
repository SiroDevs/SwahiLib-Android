...
fun HomeHistory(
    ...
    val tabs = listOf("Usomaji", "Utafutaji")
    ...
    LaunchedEffect(pagerState.currentPage) {
        viewModel.setHistorySubTab(
            if (pagerState.currentPage == 0) HomeViewModel.HistorySubTab.USOMAJI
            else HomeViewModel.HistorySubTab.UTAFUTAJI
        )
    }
...
private fun ReadingsTab(
    ...
    val rows = remember(history) {
        HistoryGrouper.group(history) { it.createdAt.toLongOrNull() }
    }
...
private fun SearchesTab(
    ...
    if (searches.isEmpty()) {
        EmptyState(
            ...
            message = "Maneno uliyotafuta hapo awali yataonekana hapa",
            messageIcon = Icons.Default.ManageSearch,
        )
        return
    }

    val rows = remember(searches) {
        HistoryGrouper.group(searches) { it.createdAt.toLongOrNull() }
    }
