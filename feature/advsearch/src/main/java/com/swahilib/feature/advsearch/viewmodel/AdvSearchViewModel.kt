package com.swahilib.feature.advsearch.viewmodel
...
class AdvSearchViewModel @Inject constructor(
    ...
    fun searchBeginningOfTerms(query: String, sortOrder: SortOrder = SortOrder.AZ) {
        ...
        _filteredSayings.value  = _allSayings.value.matchStart(q, sortOrder,
            { listOfNotNull(it.title, it.meaning) }, { it.title }, { it.liked })
    }
    ...
    fun searchMiddleOfTerms(query: String, sortOrder: SortOrder = SortOrder.AZ) {
        ...
        _filteredProverbs.value = _allProverbs.value.matchContains(q, sortOrder,
            { listOfNotNull(it.title, it.meaning, it.synonyms, it.conjugation) }, { it.title }, { it.liked })
        _filteredSayings.value  = _allSayings.value.matchContains(q, sortOrder,
            { listOfNotNull(it.title, it.meaning) }, { it.title }, { it.liked })
    }
    ...
    fun searchEndOfTerms(query: String, sortOrder: SortOrder = SortOrder.AZ) {
        ...
        _filteredProverbs.value = _allProverbs.value.matchEnd(q, sortOrder,
            { listOfNotNull(it.title, it.meaning, it.synonyms, it.conjugation) }, { it.title }, { it.liked })
        _filteredSayings.value  = _allSayings.value.matchEnd(q, sortOrder,
            { listOfNotNull(it.title, it.meaning) }, { it.title }, { it.liked })
    }
    ...
    fun addToHistory(itemId: Int, type: String) = viewModelScope.launch {
        historyRepo.saveHistory(
            HistoryEntity(item = itemId, type = type, createdAt = System.currentTimeMillis().toString())
        )
    }
    ...
    private fun <T> List<T>.replace(rid: Int, updated: T): List<T>
        ...
        (it as? SayingEntity)?.rid == rid) updated else it }
